...
def get_train_test(data, test_size=.3):
    ...
    train, test = np.float64(train.as_matrix()), np.float64(test.as_matrix())
...
def cm_models(data, models, pca=False, n_pca=2):
    
    train, train_target, test, test_target = get_train_test(data)
...
def get_classifier_stats(data, models, n=1000, pca=False, n_pca=2):
    ...
    for i in range(n):
        ...
        r = r.append(cm_models(bdata, models, pca=pca, n_pca=n_pca), ignore_index=True)
...
def get_roc_curve(data, classifier):
